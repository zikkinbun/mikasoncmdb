...
class Deploy(APIView):
    ...
    def saltApi_config_dir(self, cmds, target, record):
        ...
        record['type'] = 'node'
        record['status'] = '成功'
        record['comment'] = record['project_name'] + u' 项目部署成功'
        new_record = self.create_record(record)
        msg = {
            'retcode': 0,
            'retmsg': record['project_name'] + u' 项目部署成功',
        }
        return msg
...
class Rollback(APIView):
    ...
    def clone_checkout_tag(self, record, url, package_dir, project_dir):
        lcd = os.chdir(package_dir)
        arg = 'git clone ' + url
        clone = os.popen(arg).readlines()
        if os.path.exists(project_dir):
            os.chdir(project_dir)
            arg = 'git checkout -b ' + 'rollback' + ' ' + record['deploy_tag']
            checkout = os.popen(arg)
            msg = {
                'retcode': 0,
                'retmsg': 'success'
    ...
    def tar_package(self, record, project_dir, tarfile_path, filename, env):
        if os.path.exists(project_dir):
            shutil.move(project_dir, project_dir + '_' + env +'_' + 'rollback' + '_' + record['deploy_tag'])
            tag_dir = project_dir + '_' + env + '_' + 'rollback' + '_' + record['deploy_tag']
            # print tag_dir
            # print os.path.exists(tag_dir)
            if os.path.exists(tag_dir):
                os.chdir(tarfile_path)
    ...
    def saltApi_config_dir(self, cmds, target, record):
        ...
        record['type'] = 'node'
        record['status'] = '成功'
        record['comment'] = record['project_name'] + u' 项目回滚成功'
        new_record = self.create_record(record)
        msg = {
            'retcode': 0,
            'retmsg': record['project_name'] + u' 项目回滚成功',
        }
        return msg
    ...
    def post(self, request, format=None):
        ...
        if env == 'test':
            ...
            dirname = project + '_test_' + 'rollback' + '_' + tag
            filename = project + '_test_' + 'rollback' + '_' + tag + '_' + time.strftime("%Y%m%d")
            tarfilename = project + '_test_' + 'rollback' + '_' + tag + '_' + time.strftime("%Y%m%d") + '.tar.gz'
            record = {
                'project_name': project,
                'project_owner': owner,
                'project_env': 'TEST',
                'project_type': type,
                'deploy_branch': 'rollback',
                'deploy_tag': tag
            }
            cmds = self.reset_configfile(filename, '/home/wwwroot/releases/', configfile, env)
            # print cmds
            ...
            dirname = project + '_prod_' + 'rollback' + '_' + tag
            filename = project + '_prod_' + 'rollback' + '_' + tag + '_' + time.strftime("%Y%m%d")
            tarfilename = project + '_prod_' + 'rollback' + '_' + tag + '_' + time.strftime("%Y%m%d") + '.tar.gz'
            record = {
                'project_name': project,
                'project_owner': owner,
                'project_env': 'PROD',
